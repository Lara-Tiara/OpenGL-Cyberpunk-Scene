...
void updateArmHierarchy(
    ...
    float /*t*/)
{
    // 取消动画：将所有骨骼矩阵设为单位矩阵（绑定姿态）
    // 着色器将使用 finalBoneMatrix = I * offset，从而保持模型静止（bind pose）
...
int main()
{
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // 对面店启用发光动画（使用已有 emissiveTexture 通道）
        ...
        // 确保 VMModel 使用主 shader（当前 ourShader 已在使用状态）
